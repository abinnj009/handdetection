...
/* extraction test */
Mat diffImage,currentImage,backgroundImage;
backgroundImage = image1;
if(!backgroundImage.data )                              // Check for invalid input
    {
        cout <<  "Could not open or find the image" << std::endl ;
        return -1;
    }
sleep(5);
cap >> currentImage  ;
if(! currentImage.data )                              // Check for invalid input
    {
        cout <<  "Could not open or find the image" << std::endl ;
        return -1;
    }
...
first_leaf_start:
	...
	imwrite("/home/abin/test.png", foregroundMask);
	return 1;
}
